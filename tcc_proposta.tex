...
\begin{abstract}{COLREGS, unmanned surface vehicle, closest point of approach}
	\frm[inline]{Resumo e abstract!}
A vessel that moves autonomously is denominated Unmanned Surface Vehicle (USV). The rules for vessels in colision situations to avoid accident is defined by the COLlision REGulations at Sea (COLREGS). Then, the main goal of this work is to make an existent COLREGS compliant USV system able to identify if an encounter with another vessel will result in collision. The detection will be made through the Closest Point of Approach (CPA) technique. Set that, the Velocity Obstacle (VO) algorithm will be adapted to the system used in this work under the hypothesis of increases the system's security.
\end{abstract}
...
%----------------------------------------------------------------
\include{acronimos}
\include{tcc_proposta_cap1}
\include{tcc_proposta_cap2}
\include{tcc_proposta_cap3}
